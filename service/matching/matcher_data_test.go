...
func (s *MatcherDataSuite) TestOrder() {
	...
	tf := newPollForwarderTask(pollForwarderPriority)
...
func (s *MatcherDataSuite) TestPollForwardSuccess() {
	...
	go func() {
		...
		defer cancel()
		tres := s.md.EnqueueTaskAndWait([]context.Context{ctx}, newPollForwarderTask(pollForwarderPriority))
		...
		for {
			...
			case 5: // add poll forwarder
				...
				go func() {
					...
					res := md.EnqueueTaskAndWait(nil, newPollForwarderTask(pollForwarderPriority))
