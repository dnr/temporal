...
func (s *cmSketch) ensureByIndexes(indexes []int, target int64) (skips int) {
	...
	if offset > math.MaxUint32 {
		...
		// slide enough to leave ~50% headroom for future growth.
		s.slideBase(offset - math.MaxUint32/2)
...
func (s *cmSketch) slideBase(delta int64) {
	...
	s.base += delta
	// delta fits in uint32 since offset was > MaxUint32 and we're sliding to MaxUint32/2
	udelta := uint32(delta)
	for i := range s.cells {
		if s.cells[i] > udelta {
			s.cells[i] -= udelta
